...
int obtenerValor(Proceso& p, const string& operando) {
    ...
    else if (operando == "BX") return p.bx;
    else if (operando == "CX") return p.cx;
    else {
        try {
...
void ejecutarInstruccion(Proceso& p) {
    if (p.pc >= (int)p.instrucciones.size()) return;
    ...
    if (comando == "ADD") {
        ...
        p.pc = stoi(destino);
        return; // No incrementar PC al final
    }
    else if (comando == "NOP") {
        // No hacer nada
    }
